...
public class MinuteGateWIFIPassengerCSVLoader {
    ...
    public static void main(String[] args) {
        try{
            ...
            //aggregate the passenger count based on checkin time, so the key would be like "2016-09-13 00:47:00"
            List<String> checkinLines = IOUtils.readLines(new ClassPathResource("/airport/wifiMinute/MinuteWIFIAggregate.csv").getInputStream());
            for(String line : checkinLines) {
                String[] parts = line.split(",");
                String key = parts[0].substring(0,2) + "%" + parts[1];
                int passengerAmount = Integer.parseInt(parts[2].toString());
                if(gatePassenger.keySet().contains(key)){
